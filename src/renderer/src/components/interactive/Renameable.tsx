 ...
 */
export function Renameable( props: any ) {
        ...
        <div className={``} onDoubleClick={() => {
            ...
            {isRenaming ? (
                <input
                    ...
                    className={ `bg-transparent outline-none font-satoshi min-w-0 w-min appearance-none border-none ${ props.className ?? '' }` }
                    value={value}
                />) : (
                 <div className={ ( props.className ?? '' ) + ' font-satoshi' }
                      onClick={ props.onClick }>{ value }</div>
             )}
        </div>
    );
}
